...
output_file_dir = 'middlefile/step2.gene_groups/'
tree_dir = 'middlefile/step3.clustering_tree/'
out_put = 'middlefile/step3.POA_result.fasta'
result_out = 'middlefile/step3.genefusion.csv'
    ...
    def transcripts_and_genes(self):
        ...
        for samline in self.samlines:
            if samline.transcript != 0:                
                if samline.start > samline.transcript.start + 30 or samline.end < samline.transcript.end - 30:
...
class Tree_leaf():
    ...
    def front_bp_pos(self, samline):
        for exon in samline.transcript.exons[:-1]:
            if exon.end - 30 <= samline.end and exon.end + 30 >= samline.end:
                return samline.end - exon.end + exon.rend
        return False
    def behind_bp_pos(self, samline):
        for exon in samline.transcript.exons[1:]:
            if exon.start - 30 <= samline.start and exon.start + 30 >= samline.start:
...
'''
def read_sam_file(thread, ref_index, readfile):
    global reads
    minimap2_view_process = subprocess_popen(shlex.split('minimap2 -ax map-ont -t {} {} {}'.format(str(thread), ref_index, readfile)))
    old_id = ''
    n = 0
    for line in minimap2_view_process.stdout:
        if 'SA:' in line and '@' not in line:
            ...
            if read_flag != 4 and read_flag != 256:
                ...
                if read_id != old_id:
                    ...
                    if n > 1 and len(read.samlines) > 1:
                        if read_to_transcript(read, 0.5) == True:
                            reads.append(read)
...
def gene_groups():
    ...
    m = 0
    print('###########################################')
    print('step1.3: Filtering SA signal')
    gene_groups = []
        ...
        if index0.size > 0:
            for j in range(count[i + 1] - count[i]):
                if data0[j] > 1 and index0[j] > i:
                    ...
                    m += 1
    time2 = time.time()
    print('found ', m, ' groups with reads number >= 2')
...
def clustering(genegroups, tree_dir, out_put, result_out, if_output):
    time1 = time.time()
    print('###########################################')
    print('step1.4: Clustering the breakpoint and POA')
    k = 200
    nodes0 = []
    n_list = []
            ...
            if group.flag == True:
                for flag, tree in group.trees.items():
                    ...
                    for node in nodelist:
                        ...
                        read_list = []
                        for leaf in node.leaves:
                            sequence = leaf.fsam.sequence + leaf.bsam.sequence
                            if leaf.anti == True:
    ...
    with open(result_out, 'w') as file:
        file.write('gene1 name\tgene2 name\tgene1 id\tgene2 id\tchr1\tchr2\tgene1 pos\tgene2 pos\tbreakpoint1\tbreakpoint2\tnum\tcode\n')
        i = 0
        for node in nodes0:
            code = str(n_list[i])
            i += 1
...
'''
def control_thread(annotation, ref_index, readfile, thread, if_output, link_tuple):
    global index
    global reads
    output_file_dir, tree_dir, out_put, result_out = link_tuple
    cpu = os.cpu_count()
    if cpu < thread:
        thread = cpu
    time1 = time.time()
    print('###########################################')
    print('step1.1: start reading index file')
    ...
    time1 = time.time()
    print('###########################################')
    print('step1.2: DNA-like alignment')
    read_sam_file(thread, ref_index, readfile)
    time2 = time.time()
    ...
    clustering(genegroups, tree_dir, out_put, result_out, if_output)
    pass
'''
fuction: main
'''
def main(readfile, index_dir, middlefile, thread, if_output):
    ...
    link_tuple = (output_file_dir, tree_dir, out_put, result_out)
    time1 = time.time()
    control_thread(annotation, ref_index, readfile, thread, if_output, link_tuple)
    time2 = time.time()
    timeuse = time2 - time1
    print('Step1 totally used ' + str(timeuse) + 's')
    print('###########################################')
'''
main fuction
'''
if __name__ == "__main__":
    ...
    time1 = time.time()
    control_thread(annotation, ref_index, readfile, thread, True, link_tuple)
    time2 = time.time()
    timeuse = time2 - time1
