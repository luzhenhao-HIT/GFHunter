...
# 新增：读取 ctat-LR-fusion
def read_ctat(tp, n):
    # 兼容可能的文件名后缀
    try:
        file = open(path + '/result/Non-tumor-data/ctat-LR-fusion_' + tp + '.tsv')
    except FileNotFoundError:
        file = open(path + '/result/Non-tumor-data/ctat-LR-fusion.' + tp + '.tsv')
    ...
    while line:
        ...
        if len(parts) >= 8:
            ...
            key = create_key(g1, g2)
            if num >= n:
                ctat[key] = GeneFuison(g1, g2, bp1, bp2, 0)
        line = file.readline()
    return ctat
...
# 新增：ctat_eva 评估
def ctat_eva(tp, result):
    genefusion = read_ctat(tp, d[tp])
    eva, keys = calculate(result, genefusion)
...
def evaluation():
    ...
    for tp in tps:
        ...
        with open(outputdir + 'non-tumor ' + tp + '.csv', 'w') as f:
            ...
            # 写入 ctat 结果及获取 FP keys
            ct, k6 = ctat_eva(tp, result)
            f.write(ct)
            ...
            for key in k:
                ...
                if key in k5:
                    n.append(5)
                if key in k6: # 增加 ctat
                    n.append(6)
                keys[key] = n
            
            # n 列表从 5 个 0 扩展到 6 个 0，分别统计被 1个, 2个... 6个 工具共同检出的 FP 数量
            n = [0, 0, 0, 0, 0, 0] 
